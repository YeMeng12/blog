<template>
  ...
  <div class="blogs">
    <div class="content-header">
      <h1 style="text-align: left; color: black">博客管理<small>收藏夹</small></h1>
...
<!--    <div  class="m-container-small m-padded-tb-big">-->
      ...
      <div class="ui container">
        <el-table size="middle" current-row-key="id" :data="dataList" stripe highlight-current-row>
          ...
          <el-table-column prop="blogId" label="博客序号" align="center"></el-table-column>
          <el-table-column prop="title" label="标题" align="center"></el-table-column>
          <el-table-column prop="description" label="摘要" align="center"></el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      formData: {}, // 表单数据
      // dataList: [{"title": "biaoti", "blogId": "1", "description":"摘要", "time": "chuangjianshij"}],// 当前页要展示的分页列表数据
      dataList: [],
      rules: { // 校验规则
        // 真实姓名
        name: [
  ...
  methods: {
    ...
    async findPage () {
      ...
      const { data: res } = await this.$axios.get('/api/user/getFavorite/' + this.$store.getters.getUserInfo.id)
      if (res.code !== 200) {
    ...
    cancelFavorites (blogId) {
      const { data: res } = this.$axios.get(`/api/server/blog/favorite/${blogId}/${this.uid}`)
      if (res.flag) {
