<template>
  <div class="blogs">
    <div class="content-header">
      <h1 style="text-align: left; color: black">用户管理<small>个人信息</small></h1>
...
export default {
  ...
  created () {
    this.formData.uid = this.$store.state.userInfo.uid
    this.formData.nickname = this.$store.state.userInfo.username
  ...
  methods: {
    ...
    async uploadPic (param) {
      ...
      const { data: res } = await this.$axios.post('/serverApi/file/userAvatar', form)
      if (res.flag) {
        // 弹出提示信息
    ...
    submitForm () {
      ...
      this.$refs.userForm.validate((valid) => {
        if (valid) {
          ...
          this.$axios.put('/api/server/user/admin/updateUser', param).then((res) => {
            // 关闭新增窗口
            if (res.data.flag) {
              // 弹出提示信息
              this.$message({
